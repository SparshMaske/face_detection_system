...
class Config:
    ...
    DEFER_EVENT_RECOGNITION = os.getenv('DEFER_EVENT_RECOGNITION', '1').strip().lower() in ('1', 'true', 'yes', 'on')
    EVENT_CAPTURE_INTERVAL_SEC = float(os.getenv('EVENT_CAPTURE_INTERVAL_SEC', 0.85))
    EVENT_MAX_CAPTURES = int(os.getenv('EVENT_MAX_CAPTURES', 99999))
    EVENT_CLUSTER_THRESHOLD = float(os.getenv('EVENT_CLUSTER_THRESHOLD', 0.60))
